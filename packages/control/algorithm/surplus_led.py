...
from control.algorithm.filter_chargepoints import get_chargepoints_by_mode_and_counter, get_preferenced_chargepoint_with_set_current_and_charging, get_chargepoints_pv_charging, get_chargepoints_surplus_led
...
class SurplusLed:
    ...
    def _set(self, chargepoints: List[Chargepoint], feed_in_yield: Optional[int], mode_tuple: Tuple[Optional[str], str, bool], counter: Counter):
        common.update_raw_data(chargepoints)
        ...
        for i in range(chargepoints_num):
            ...
            available_currents, limit = Loadmanagement().get_available_currents(missing_currents, counter, feed_in_yield)
            available_for_cp = common.available_current_for_cp(chargepoints[-1], counts, available_currents)
            current = common.get_current_to_set(
                chargepoints[-1].data.set.current, available_for_cp, chargepoints[-1].data.set.target_current)
            self._set_loadmangement_message(current, limit, chargepoints[-1], counter)
            limited_current = self._limit_adjust_current(chargepoints[-1], current)
            common.set_current_counterdiff(
                limited_current - chargepoints[-1].data.set.charging_ev_data.ev_template.data.min_current,
                limited_current,
                chargepoints[-1])
            chargepoints.pop(-1)
    ...
    def _set_loadmangement_message(self,
        ...
        if (current != chargepoint.data.set.current and
                ...
                chargepoint.data.set.current != max(chargepoint.data.set.charging_ev_data.data.control_parameter.required_currents) and
    ...
    def filter_by_feed_in_limit(self, chargepoints: List[Chargepoint]) -> Tuple[List[Chargepoint], List[Chargepoint]]:
        cp_with_feed_in = list(filter(lambda cp: cp.data.set.charging_ev_data.charge_template.data.chargemode.pv_charging.
                                      feed_in_limit is True, chargepoints))
        cp_without_feed_in = list(filter(lambda cp: cp.data.set.charging_ev_data.charge_template.data.chargemode.pv_charging.
                                         feed_in_limit is False, chargepoints))
    ...
    def check_switch_on(self) -> None:
        for cp in get_chargepoints_pv_charging():
            if cp.data.set.current == 0 and cp.data.set.charging_ev_data.data.control_parameter.timestamp_switch_on_off is None:
        ...
        for cp in get_chargepoints_surplus_led():
            ...
            if charging_ev_data.data.control_parameter.phases == 1:
                charging_ev_data.data.control_parameter.required_currents = [
                    charging_ev_data.ev_template.data.max_current_one_phase if required_currents[i] != 0 else 0 for i in range(3)]
            else:
                charging_ev_data.data.control_parameter.required_currents = [
