...
class Algorithm:
    ...
    def calc_current(self) -> None:
        ...
        try:
            ...
            if self.evu_counter.data["set"]["raw_power_left"] > 0:
                log.debug("**PV-geführten Strom setzen**")
                self.surplus_led.reset_current_to_target_current()
    ...
    def _check_auto_phase_switch_delay(self) -> None:
        ...
        möglich ist und ob ob ein Timer gestartet oder geChargemode.STOP.valuept werden muss oder ob
        ...
        for cp in data.data.cp_data.values():
            try:
                if cp.data.set.charging_ev != -1:
                    ...
                    pv_auto_switch = (control_parameter.chargemode == Chargemode.PV_CHARGING.value and
                                      data.data.general_data.get_phases_chargemode(Chargemode.PV_CHARGING.value) == 0)
                    scheduled_auto_switch = (
                        control_parameter.chargemode == Chargemode.SCHEDULED_CHARGING.value and
                        control_parameter.submode == Chargemode.PV_CHARGING.value and
