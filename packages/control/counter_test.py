...
@pytest.mark.parametrize("raw_currents_left, expected_max_exceeding",
                         [pytest.param([32, 32, 15], [0]*3, id="unbalanced load limit not exceeded"),
                          ...
                          pytest.param([35, 11, 8], [0, 4, 7], id="unbalanced load limit exceeded on two phases with export")])
...
@pytest.mark.parametrize("hierarchy, max_currents, expected_raw_currents_left",
                         [pytest.param(hierarchy_standard, [40]*3, [40, 0, 10], id="Überbelastung"),
                          (hierarchy_standard, [60]*3, [60, 15, 30]),
                          (hierarchy_hybrid, [60]*3, [60, 15, 30]),
                          (hierarchy_nested, [60]*3, [60, 15, 30])])
