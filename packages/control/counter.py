...
class Counter:
    ...
    def __init__(self, index):
        try:
            self.data = {"set": {"reserved_surplus": 0, "released_surplus": 0, "raw_power_left": 0},
                         "get": {
                "daily_exported": 0,
    ...
    def calc_surplus(self):
        ...
        reserved_surplus = self.data["set"]["reserved_surplus"]
        raw_power_left = evu_counter.data["set"]["raw_power_left"]
        max_power = evu_counter.data["config"]["max_total_power"]
        surplus = raw_power_left - max_power + bat_surplus - reserved_surplus
    ...
    SWITCH_ON_NOT_EXCEEDED = "Die Ladung kann nicht gestartet werden, da die Einschaltschwelle {}W nicht erreicht wird."
    ...
    def switch_on_threshold_reached(self, chargepoint: Chargepoint):
        try:
            surplus = self.data["set"]["raw_power_left"] - self.data["set"]["reserved_surplus"]
            message = None
            control_parameter = chargepoint.data.set.charging_ev_data.data.control_parameter
            required_power = chargepoint.data.set.charging_ev_data.ev_template.data.min_current * control_parameter.phases * 230
            ...
            if timestamp_switch_on_off != control_parameter.timestamp_switch_on_off:
                ...
                Pub().pub(
                    f"openWB/set/vehicle/{chargepoint.data.set.charging_ev_data.num}/control_parameter/timestamp_switch_on_off", timestamp_switch_on_off)
        ...
        try:
            if charging_ev.data.control_parameter.timestamp_switch_on_off is not None:
                ...
                if not chargepoint.data.get.charge_state:
                    data.data.counter_all_data.get_evu_counter().data["set"]["reserved_surplus"] -= pv_config.switch_on_threshold * \
                        chargepoint.data.set.phases_to_use
                    log.debug("reserved_surplus 10 " +
                              str(data.data.counter_all_data.get_evu_counter().data["set"]["reserved_surplus"]))
                else:
                    data.data.counter_all_data.get_evu_counter().data["set"]["released_surplus"] -= pv_config.switch_on_threshold * \
                        charging_ev.data.control_parameter.phases
                    log.debug("reserved_surplus 11 " +
                              str(data.data.counter_all_data.get_evu_counter().data["set"]["reserved_surplus"]))
        except Exception:
            log.exception("Fehler im allgemeinen PV-Modul")
...
def limit_raw_power_left_to_surplus(surplus) -> None:
    for counter in data.data.counter_data.values():
        ...
        counter.data["set"]["raw_power_left"] = surplus
        log.debug(
            f'Zähler {counter.num}: Begrenzung der verbleibenden Leistung auf {counter.data["set"]["raw_power_left"]}W')
